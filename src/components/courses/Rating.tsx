...
function Rating({ review, refetchAllReviews }: RatingProps) {
  ...
  const handleDeleteRating = () => {
    const cf = window.confirm('Confirm deletion?');
  ...
  useEffect(() => {
    if (status === 'success') {
      toast.success('Clear review successful!');
      refetchAllReviews();
    }
  ...
  return (
    <li className="flex min-h-[15rem] w-full flex-col items-center justify-center rounded-2xl bg-light-background px-4 py-2 dark:bg-background_dark">
      ...
      <div className={`mt-2 flex w-full flex-col p-2`}>
        ...
        >
          ...
          {isOverflow && (
            ...
            >
              <span>{showMore ? 'Collapse' : 'See more'}</span>{' '}
              <ChevronDownIcon
